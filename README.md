...
## 25. 12. 05(금)
### 오늘의 질문: 마이크로서비스 아키텍처(Microservices Architecture)가 무엇이고, 모놀리식 아키텍처와 어떤 차이가 있나요? 각각의 장단점도 설명해주세요.

### 1. 모놀리식 아키텍처(Monolithic Architecture)란?

**서비스 전체 기능을 하나의 큰 애플리케이션으로 구성하는 방식**

예를 들어 쇼핑몰 서비스를 만든다면 하나의 애플리케이션 안에
...
작은 수정에도 전체 애플리케이션 빌드·배포 필요

특정 기능 하나 때문에 전체 서비스 영향 가능

트래픽 증가 시 부분 확장이 불가능

팀 규모가 커지면 충돌·개발 속도 저하

### 2. 마이크로서비스 아키텍처(MSA)란?

**서비스를 여러 개의 작은 서비스 단위로 쪼개서 독립적으로 개발하고 배포하는 구조**

각 기능을 별도 애플리케이션으로 나누는 것:
...
각 서비스는 독립적인 코드, 독립적인 DB, 독립적인 배포를 가짐.

✔ MSA의 장점
1) 독립적 배포 가능 → 개발 속도 ↑

한 서비스만 고쳐도 독립적으로 배포 가능

→ 전체 서비스에 영향 없이 빠른 배포 가능
...
→ Product Service만 Scale-out 가능 → 비용 절감

3) 장애 격리 (Fault Isolation)

어떤 서비스가 죽어도 전체 서비스가 죽지 않음
예:
리뷰 서비스 죽어도 주문·결제는 정상 동작
...
서비스 간 통신(REST, gRPC), 메시징, 장애 처리 등 고려해야 할 요소 증가

2) 분산 시스템의 어려움 존재

* 네트워크 지연
* 장애 전파
* 트랜잭션 처리 어려움(2PC, Saga 패턴 필요)
* 데이터 정합성 확보 문제
...
서비스 수 증가 → 서버/컨테이너도 증가
→ 비용 상승 가능성

### 3. 모놀리식 vs MSA 비교 한눈에 보기

| 항목       | 모놀리식         | 마이크로서비스(MSA) |
| -------- | ------------ | ------------ |
| 구조       | 하나의 큰 애플리케이션 | 여러 개의 독립 서비스 |
| 배포       | 전체 재배포 필요    | 서비스 단위 개별 배포 |
| 확장성      | 전체를 같이 확장    | 서비스별 부분 확장   |
| 장애 영향    | 전체 서비스 영향    | 장애 격리 가능     |
| 코드베이스    | 하나           | 여러 개         |
| 데이터베이스   | 하나(DB 공유)    | 서비스별 독립 DB   |
| 복잡도      | 낮음           | 높음           |
| 초기 개발 속도 | 빠름           | 느림           |
| 운영/모니터링  | 쉽다           | 어렵다(도구 필요)   |
| 팀 규모     | 소규모 적합       | 대규모 적합       |

### 결론 (한 문장 요약)

모놀리식은 단순하고 빠르게 개발 가능하지만 확장성과 릴리즈 유연성이 떨어지고,
MSA는 복잡하지만 확장성·독립 배포·팀 분업에 최적화된 현대적 아키텍처이다.


## 25. 12. 11(목)
### 오늘의 질문: WebSocket이 무엇이고, 기존 HTTP 통신과 어떤 차이가 있나요? 어떤 상황에서 WebSocket을 사용하나요?

### 1. WebSocket이란?

**클라이언트와 서버가 하나의 연결을 유지하면서
실시간으로 양방향 통신을 할 수 있게 해주는 프로토콜**
...
**(1) HTTP는 요청(Request) 기반 — 단방향 구조**

클라이언트가 요청해야 서버가 응답

서버는 마음대로 먼저 보내지 못함

매 요청마다 연결 생성 → 응답 → 연결 종료

즉, “필요할 때마다 끊었다 다시 연결”하는 방식.

**(2) WebSocket은 연결을 유지하는 양방향 구조**

한 번 연결(Handshake) 후 계속 유지(Persistent Connection)

서버가 클라이언트에게 먼저 메시지 Push 가능

데이터 오갈 때 Header가 작아 오버헤드 ↓

실제 실시간 통신에 적합

비유하자면:

HTTP: 손님이 “물 좀 주세요”라고 말해야만 웨이터가 물을 줌

WebSocket: 웨이터가 알아서 필요한 때에 물, 음식, 공지 등을 실시간으로 바로 전달 가능

### 3. WebSocket 통신 흐름

클라이언트 → 서버: WebSocket 연결 요청 (HTTP Upgrade 요청)

서버: Upgrade 승인

이제 둘은 WebSocket 프로토콜로 전환됨

이후 양방향 메시지를 마음대로 주고받음

### 4. 어떤 상황에서 WebSocket을 사용할까? (필수 사용 케이스)
✔ 1) 실시간 채팅(대표적인 예)

카카오톡, 메신저, 웹 채팅

→ 클라이언트가 요청하지 않아도 서버가 메시지를 바로 push
...
### 6. 결론 — 언제 WebSocket을 쓸까?

**“실시간 양방향 통신이 필요하다면 WebSocket,
요청-응답 기반이면 HTTP”**
